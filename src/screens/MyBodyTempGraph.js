...
export default class MyBodyTempGraph extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      userrole: "",
  ...
  async ReportDetailCall() {
    try {
      const response = await axios.get(Constants.GET_TEMP);
      ...
      if (response.data.Status) {
        ...
        response.data.List.map((item) => {
          ...
          let temp = {};
          let temp1 = {};
          let str = Number(item.Name).toFixed(1);
          ...
          tempreport.systolic = str;
          tempreport.diastolic = far.toFixed(1);
  ...
  showGraphUsingDates = async () => {
    ...
    // );
    try {
      let response = await axios.post(Constants.GET_TEMP_BYDATES, {
