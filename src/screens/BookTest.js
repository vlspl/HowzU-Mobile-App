...
export default class BookTest extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps = (nextprops) => {
    ...
    if (nextprops.route.params.from == "manually") {
      ...
      let count = nextprops.route.params.Testids.replace(
        /(^,)|(,$)/g,
        ""
      ).split(",").length;
  ...
  componentDidMount = () => {
    ...
    // );
    // console.log(
    //   'sugessted test Booking  componentDidMount==============================',
    ...
    if (this.props.route.params.from == "manually") {
      ...
      // {"BookStatus": 1, "DoctorId": "782", "DoctorName": "Raju Gate", "LabId": "3", "LabName": "Sun Lab", "RecommendationId": 30, "RecommendedDate": "04/03/2020 14:56:01 PM", "TestName": "Peripheral Smear Examination,Adenosine Deaminase, Blood,Sputum Culture,Pus Culture", "TestPrice": "0,0,0,0", "Total": 0, "testId": "182,204,236,237"}
      ...
      //   this.props.route.params.labinfo.RecommendationId
      // );
      // console.log(
      //   ' suggested test prices ==============================',
  ...
  Getlabdataslot = async () => {
    ...
    try {
      ...
      // Toast.show(
      //   "Network Error,Plase check you're internet connection or  try again later"
      // );
  ...
  suggestedBookAppointments = async () => {
    //  console.log("-------Sugeested Book Appointment", this.state.RcomId);
    let PaymentMethod = "";
    PaymentMethod = this.state.isonlinePayment ? "Online" : "";
    let testprice =
      this.state.testidprices.replace(/(^,)|(,$)/g, "") == null
        ? 0
